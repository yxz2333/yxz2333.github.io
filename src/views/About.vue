  ...
  <div class="min-h-screen text-gray-300 font-sans relative">
    <main class="relative z-10 max-w-[88rem] mx-auto px-4 lg:px-6 pt-32 pb-24">
      ...
      <div class="mb-12 animate-fade-in-up">
        <h1 class="text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-br from-white to-gray-500 tracking-tight mb-2">
          关于我
        </h1>
      ...
      <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-12 gap-6">
          ...
          <!-- ① 个人名片 (3列) -->
          <div class="lg:col-span-3 glass-card rounded-[2rem] p-6 flex flex-col items-center justify-center text-center animate-fade-in-up" style="animation-delay: 0.1s">
            <div class="relative w-20 h-20 mb-4">
              <div class="absolute inset-0 bg-accent rounded-full blur-md opacity-30"></div>
              <img v-if="profile.basic.avatar" :src="profile.basic.avatar" class="relative w-full h-full rounded-full object-cover border-2 border-white/10" alt="avatar" />
              <span v-else class="relative w-full h-full rounded-full bg-[#1c1c1e] border-2 border-white/10 flex items-center justify-center text-2xl font-bold text-white">{{ profile.basic.name[0] }}</span>
              <div class="absolute -bottom-1.5 -right-1.5 bg-[#121214] border border-white/10 px-2 py-0.5 rounded-full text-[10px] font-mono text-accent shadow-lg">
          ...
          <!-- ② 教育 (3列) -->
          <div class="lg:col-span-3 glass-card rounded-[2rem] p-6 flex flex-col items-center justify-center text-center animate-fade-in-up" style="animation-delay: 0.15s">
            <div class="w-10 h-10 rounded-xl bg-accent/10 border border-accent/20 flex items-center justify-center text-accent mb-3 text-lg">
              🎓
            </div>
          ...
          <!-- ③ 技术栈 (6列) -->
          <div class="lg:col-span-6 glass-card rounded-[2rem] p-6 flex flex-col justify-center animate-fade-in-up" style="animation-delay: 0.2s">
            <h3 class="text-xs font-bold text-gray-500 uppercase tracking-widest mb-4">Tech Stack</h3>
            <div class="flex flex-wrap gap-2">
              <span
                v-for="tech in profile.skills" :key="tech"
        ...
        <div class="glass-card rounded-[2rem] p-8 animate-fade-in-up" style="animation-delay: 0.3s">
          <div class="flex items-center justify-between mb-6">
            ...
            <button
              v-if="profile.allAwards && profile.allAwards.length"
              @click="openTimeline()"
              class="text-xs text-gray-400 hover:text-accent transition-colors tracking-wide font-bold"
            >XCPC CAREER ({{ profile.allAwards.length }})</button>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3">
            <div
              v-for="award in profile.awards" :key="award.date + award.name"
            ...
            >
              ...
              <span class="text-xs font-bold px-2 py-0.5 rounded border shrink-0" :class="medalClass(award.medal)">{{ award.medal }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="glass-card rounded-[2rem] p-8 animate-fade-in-up" style="animation-delay: 0.4s">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <a
              v-for="proj in profile.projects" :key="proj.name"
            ...
            >
              <div class="flex justify-between items-start mb-4">
                <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-gray-800 to-black border border-white/10 flex items-center justify-center text-lg shadow-inner">
                  {{ proj.icon }}
                </div>
                <svg class="w-4 h-4 text-gray-600 group-hover:text-accent transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg>
              </div>
              ...
              <div class="flex flex-wrap gap-2">
                <span v-for="tag in proj.tags" :key="tag" class="text-[10px] font-mono text-gray-400 bg-black/30 px-2 py-0.5 rounded border border-white/5">{{ tag }}</span>
              </div>
            </a>
          </div>
        </div>
      ...
      <Transition name="dr-panel">
        ...
        >
          ...
          <div class="flex items-center justify-between px-6 py-5 border-b border-white/5 shrink-0">
            <h2 class="text-lg font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-accent to-yellow-400 tracking-wide uppercase">XCPC Career</h2>
            <button
              @click="closeTimeline()"
          ...
          <div class="flex-1 overflow-y-auto px-6 py-8">
            <div class="relative">
              ...
              <div class="space-y-8">
                <div
                  v-for="(award, i) in profile.allAwards" :key="i"
                  class="flex gap-5 group animate-fade-in-up"
                  :style="{ animationDelay: `${i * 0.05}s` }"
                >
                  <!-- 节点 -->
                  <div class="relative z-10 shrink-0 flex flex-col items-center">
                    <div
                      class="w-[8px] h-[8px] rounded-full mt-1.5 ring-[3px] ring-[#121214]"
                      :class="medalClass(award.medal).split(' ')[1] || 'bg-gray-500'"
                    ></div>
                  </div>
                  ...
                  <div class="flex-1 pb-2">
                    <div class="flex items-center justify-between gap-3 mb-1">
                      ...
                      <span class="text-[10px] font-bold px-1.5 py-0.5 rounded border shrink-0" :class="medalClass(award.medal)">{{ award.medal }}</span>
                    </div>
